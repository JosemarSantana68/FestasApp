...
namespace FestasApp.Views.TabelasAuxiliares
{
    public partial class FormItensFestasCRUD : FormBaseCRUD
    {
        ...
        private void MostrarItemFesta()
        {
            try
            {
                ...
                if (item != null)
                {
                    ...
                    var txtTipo = this.pnlCentral.Controls["txtTipo"] as TextBox;
                    ...
                    if (txtValor != null)
                    {
                        txtValor.Text = (item.itensfest_valor)?.ToString("F2") ?? "0,00";
                ...
                else
                {
                    ...
                    // myUtilities.myMessageBox(this, "Item não encontrado.");
                }
            }
            catch (Exception ex)
            {
                myUtilities.myMessageBox(this, $"Erro ao preencher dados do Item: {ex.Message}");
            }
        }
        //
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        myUtilities.myMessageBox(this, "Erro ao excluir item de festa!", "Item Excluir", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
        ...
        private void SetControls()
        {
            ...
            //int maxLenghtValor = myFunctions.GetMaxLength("tblfestasitens", "itensfest_valor"); // decimal(10,2) erro com -1

            Label lblNome = new Label { Text = "Nome:", Location = new Point(lblCol, rowControl) };
            myTextBox txtNome = new myTextBox { Location = new Point(txtCol, rowControl), Name = "txtNome", Width = larguraNome };

            rowControl += 30;
            Label lblTipo = new Label { Text = "Tipo:", Location = new Point(lblCol, rowControl) };
            myTextBox txtTipo = new myTextBox { Location = new Point(txtCol, rowControl), Name = "txtTipo", Width = larguraTipo };

            rowControl += 30;
            Label lblDescricao = new Label { Text = "Descrição:", Location = new Point(lblCol, rowControl) };
            myTextBox txtDescricao = new myTextBox { Location = new Point(txtCol, rowControl), Name = "txtDescricao", Width = larguraDescricao };

            rowControl += 30;
            Label lblValor = new Label { Text = "Valor:", Location = new Point(lblCol, rowControl) };
            myTextBoxNumericos txtValor = new myTextBoxNumericos { Location = new Point(txtCol, rowControl), Name = "txtValor", Width = larguraValor, TextAlign = HorizontalAlignment.Right, MyPermitirZerado = true };
        ...
        private void SalvarItensFestas()
        {
            ...
            {
                myUtilities.myMessageBox(this, "Por favor, preencha todos os campos.", "Itens de Festa");
                return;
            }
            ...
            {
                myUtilities.myMessageBox(this, "Por favor, insira um valor válido.", "Itens de Festa");
                return;
            }

            // Cria uma instância da classe clsFestasItens e salva os dados sem passar o ID
            clsFestasItens item = new clsFestasItens(tipo, nome, descricao, valor);
            ...
            try
            {
                if (operacao == OperacaoCRUD.NOVO)
                {
                    ...
                    if (repItensFestasEF.AdicionarItemFesta(item))
                    {
                        myUtilities.myMessageBox(this, "Item de Festas adicionado com sucesso!", "Itens de Festas");
                        this.Close();
                    }
                    else
                    {
                        myUtilities.myMessageBox(this, "Falha ao adicionar o Item de Festas.", "Itens de Festas");
                    }
                }
                else if (operacao == OperacaoCRUD.EDITAR)
                {
                    ...
                    {
                        myUtilities.myMessageBox(this, "Item de Festas alterado com sucesso!", "Itens de Festas");
                        this.Close();
                    }
                    else
                    {
                        myUtilities.myMessageBox(this, "Falha ao alterar o Item de Festas.", "Itens de Festas");
                    }
                }
